...
def print_analog_group(word):
    ...
    for analog_output_idx, analog_output in word['AnalogGroup']['ChannelDatas'].items():
        ...
        if len(analog_output['waveform']['XValues']) == 1:
            if analog_output['waveform']['XValues'][0]['ParameterValue'] != 0:
                raise ValueError(
...
def generate_analog_code(analog_dict, chl_names):
    with open(r'temp\autogenerated.py', 'a') as f:
...
with open(r'temp\autogenerated.py', 'w') as f:
    f.write(f'#autogenerated from {filename}\n')
    f.write('start()\n')
    f.write('t=0\n')

# remove disabled words
my_list = [word for word in my_list if word['StepEnabled']]
